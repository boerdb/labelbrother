#!/usr/bin/env python3
"""Eerste keer: git clone op NEXT-server, build, PM2 (poort 3022)."""
...
REPO = os.environ.get(
    "DEPLOY_REPO",
    "git@github.com:boerdb/labelbrother.git",
)
...
def main() -> None:
    ...
    run(
        ssh,
        f"test -f {REMOTE_DIR}/.env.production || cp {REMOTE_DIR}/.env.production.example {REMOTE_DIR}/.env.production",
        check=False,
    )
